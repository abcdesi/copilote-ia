...
export function createGoogleState(companyId: string) {
...
export function verifyGoogleState(state: string) {
  ...
  const parsed = JSON.parse(Buffer.from(payload, "base64url").toString("utf8")) as { companyId: string; ts: number };
...
export function buildGoogleAuthorizationUrl(companyId: string) {
  ...
  url.searchParams.set("state", createGoogleState(companyId));
  return url.toString();
}
