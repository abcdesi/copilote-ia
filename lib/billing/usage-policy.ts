...
export async function getUsageStatus(companyId: string) {
  const [company, subscription, trialStartEvent, usageEvents] = await Promise.all([
    ...
    prisma.event.findMany({
      where: { companyId, type: { in: ["USAGE_RESERVED", "USAGE_REFUNDED"] } },
  ...
  const creditsLimit = safeNumber(TRIAL_CREDITS, 100);
  const costCapEur = safeNumber(TRIAL_COST_CAP_EUR, 3);
...
export async function reserveUsage(input: {
  ...
  const paid = Boolean(subscription && subscription.plan !== "free" && ["active", "trialing"].includes(subscription.status));
  if (paid) return { allowed: true as const, paid: true as const, reservationId: null };
  ...
  if (!trialStart) {
    trialStart = await prisma.event.create({
      data: {
        ...
        type: "TRIAL_STARTED",
        metadata: JSON.stringify({ days: safeNumber(TRIAL_DAYS, 14), credits: safeNumber(TRIAL_CREDITS, 100), costCapEur: safeNumber(TRIAL_COST_CAP_EUR, 3) }),
      },
    });
  }

  const status = await getUsageStatus(input.companyId);
  if (!status.trialActive) return { allowed: false as const, reason: "trial_expired" as const, status };
  if (status.creditsUsed + input.credits > status.creditsLimit) return { allowed: false as const, reason: "credits_exhausted" as const, status };
  if (status.reservedCostEur + input.reservedCostEur > status.costCapEur) return { allowed: false as const, reason: "cost_cap_reached" as const, status };

  const reservation = await prisma.event.create({
    data: {
      companyId: input.companyId,
      type: "USAGE_RESERVED",
      metadata: JSON.stringify({ kind: input.kind, credits: input.credits, reservedCostEur: input.reservedCostEur }),
    },
  });
  return { allowed: true as const, paid: false as const, reservationId: reservation.id };
}
...
export async function refundUsage(input: {
  ...
  await prisma.event.create({
    data: {
      companyId: input.companyId,
      type: "USAGE_REFUNDED",
      metadata: JSON.stringify({ reservationId: input.reservationId, kind: input.kind, credits: input.credits, reservedCostEur: input.reservedCostEur }),
    },
  });
}
