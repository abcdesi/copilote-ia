...
export function assessAdviceMaturity(context: ChatContext): AdviceMaturityAssessment {
  ...
  if (context.industry) {
    score += 10;
    knownSignals.push("secteur");
  } else missingSignals.push("secteur");

  if (context.sizeRange) {
    score += 10;
    knownSignals.push("taille");
  } else missingSignals.push("taille");

  if (context.objectives) {
    score += 15;
    knownSignals.push("objectifs");
  } else missingSignals.push("objectifs business");

  if (context.painPoints) {
    score += 15;
    knownSignals.push("irritants");
  } else missingSignals.push("irritants prioritaires");

  if (context.tools.length > 0) {
    score += Math.min(10, context.tools.length * 3);
    knownSignals.push("outils déclarés");
  } else missingSignals.push("outils");

  const activeConnections = context.connections.filter((connection) => connection.status === "connected" || connection.status === "active");
  if (activeConnections.length > 0) {
    score += Math.min(15, activeConnections.length * 7);
    knownSignals.push("sources connectées");
